    ...
    <section ref={sectionRef} className="py-20 px-4 bg-black">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center mb-16">
          <p className="text-gray-400 text-sm mb-4 tracking-wide">{t.cases.sectionLabel}</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-8 mb-12">
          {casesToShow.map((caseItem, index) => (
            <div
              key={caseItem.id}
              className={`relative rounded-3xl overflow-hidden group cursor-pointer transform transition-all duration-500 ${
            ...
            >
              ...
              <div className="absolute inset-0 p-6 flex flex-col justify-end">
                <h3 className="text-white text-2xl font-bold mb-3">{caseItem.name}</h3>
                <div className="space-y-1 mb-4">
                  <p className="text-gray-200 text-sm">{caseItem.houses}</p>
                  <p className="text-orange-Globe text-sm font-semibold">{caseItem.sold}</p>
